...
const PAGE_BORDER_LEFT = 20;
const MAX_HORIZONTAL_WIDTH = 155;
const STANDARD_FONT = 'helvetica';
const TITLE_FONT = 'courier';
const TEXT_COLOR_BLACK = '#000000';
const TEXT_COLOR_LIGHT_GREY = '#868686';
...
export class PdfComponent implements OnInit {
  ...
  public download() {
    ...
    const pdf = this.generatePdfDocument();
  ...
  private addGeneralDescription(pdf: jsPDF) {
    pdf.setFontSize(14);
    pdf.setFont(STANDARD_FONT, 'bold');
    pdf.text(this.l10n.overview.subtitle_aboutme.toUpperCase(), PAGE_BORDER_LEFT, this.verticalPosition);
    this.verticalPosition += 10;
    pdf.setFontSize(12);
  ...
  private addLanguages(pdf: jsPDF) {
    if (this.languageItems !== undefined && this.languageItems.length > 0) {
      ...
      pdf.setFontSize(14);
      pdf.setFont(STANDARD_FONT, 'bold');
      pdf.text(this.l10n.education.title_languages.toUpperCase(), PAGE_BORDER_LEFT, this.verticalPosition);
      this.verticalPosition += 10;
      pdf.setFontSize(12);
      pdf.setFont(STANDARD_FONT, 'normal');
      for (const item of this.languageItems) {
        ...
        pdf.rect(PAGE_BORDER_LEFT, this.verticalPosition - 4, bar_length, 5.5, 'F');
        pdf.setDrawColor(0);
  ...
  private addInterests(pdf: jsPDF) {
    if (this.interestItems !== undefined && this.interestItems.length > 0) {
      ...
      const splittedInterests = pdf.splitTextToSize(interests, MAX_HORIZONTAL_WIDTH);
      pdf.text(splittedInterests, PAGE_BORDER_LEFT, this.verticalPosition);
    }
  }

  private addLine(pdf: jsPDF, title: string, content: string) {
    if (content !== undefined && content !== '') {
      pdf.setFontSize(12);
      pdf.setFont(STANDARD_FONT, 'bold');
      pdf.text(title, PAGE_BORDER_LEFT, this.verticalPosition);
      pdf.setFontSize(11);
      pdf.setFont(STANDARD_FONT, 'bold');
  ...
  private addExperiencePageContent(pdf: jsPDF) {
    this.addPageTitle(pdf, this.l10n.experience.title);
    this.addCvItems(pdf, this.cvItems, true);
  }

  private addProjectsPageContent(pdf: jsPDF) {
    this.addPageTitle(pdf, this.l10n.projects.title);
    this.addCvItems(pdf, this.projectItems);
  }

  private addVolunteeringPageContent(pdf: jsPDF) {
    this.addPageTitle(pdf, this.l10n.volunteering.title);
    this.addCvItems(pdf, this.volunteerItems);
  }

  private addEducationPageContent(pdf: jsPDF) {
    this.addPageTitle(pdf, this.l10n.education.title_education);
    this.addCvItems(pdf, this.educationItems);
  }

  private addCertificatesPageContent(pdf: jsPDF) {
    this.addPageTitle(pdf, this.l10n.education.title_certificates);
    this.addCvItems(pdf, this.certificationItems);
  }

  private addPublicationsPageContent(pdf: jsPDF) {
    this.addPageTitle(pdf, this.l10n.publications.title_publications);
    this.addCvItems(pdf, this.publicationItems);
  }

  private addTalksPageContent(pdf: jsPDF) {
    this.addPageTitle(pdf, this.l10n.publications.title_talks);
    this.addCvItems(pdf, this.talkItems);
  }

  private addPageTitle(pdf, title: string) {
    pdf.setFontSize(14);
    pdf.setFont(STANDARD_FONT, 'bold');
    pdf.text(title.toUpperCase(), PAGE_BORDER_LEFT, this.verticalPosition);
  ...
  private splitPageIfNecessary(pdf: jsPDF) {
    if (this.verticalPosition >= 235) {
      pdf.setTextColor(TEXT_COLOR_DARK_GREY);
      pdf.setFont(STANDARD_FONT, 'italic');
      pdf.setFontSize(10);
      this.verticalPosition += 5;
      pdf.text(this.l10n.pdf.continuing_next_page, PAGE_BORDER_LEFT, this.verticalPosition);
      this.switchPage(pdf);
      pdf.setTextColor(TEXT_COLOR_DARK_GREY);
      pdf.setFont(STANDARD_FONT, 'italic');
      pdf.setFontSize(10);
      pdf.text(this.l10n.pdf.continued_from_last_page, PAGE_BORDER_LEFT, this.verticalPosition);
      pdf.setTextColor(TEXT_COLOR_BLACK);
      pdf.setFont(STANDARD_FONT, 'normal');
  ...
  private addCvItem(pdf, item: CvItem, addVerticalLine?: Boolean) {
    ...
    pdf.setDrawColor(0);
    pdf.setFillColor(TEXT_COLOR_BLACK);
  ...
  private addCvItemTitle(pdf, item: CvItem) {
    pdf.setFontSize(12);
    pdf.setFont(STANDARD_FONT, 'bold');
    const splittedTitle = pdf.splitTextToSize(item.title, MAX_HORIZONTAL_WIDTH);
    pdf.text(splittedTitle, 30, this.verticalPosition);
    this.verticalPosition += splittedTitle.length * 5;

    pdf.setFontSize(12);
    pdf.setFont(STANDARD_FONT, 'normal');
    pdf.setTextColor(TEXT_COLOR_LIGHT_GREY);
    pdf.text(item.subtitle, 30, this.verticalPosition);
    const splittedSubtitle = pdf.splitTextToSize(item.subtitle, MAX_HORIZONTAL_WIDTH);
    pdf.text(splittedSubtitle, 30, this.verticalPosition);
    this.verticalPosition += splittedSubtitle.length * 5;
  }
  ...
  private addCvItemTags(pdf, item: CvItem) {
    ...
    if (item.tags !== undefined && item.tags.length > 0) {
      ...
      const splittedTags = pdf.splitTextToSize(tags, MAX_HORIZONTAL_WIDTH - 10);
  ...
  private addCvItemDescription(pdf, item: CvItem) {
    if (item.description !== undefined && item.description !== '') {
      ...
      pdf.setFont(STANDARD_FONT, 'italic');
      pdf.setTextColor(TEXT_COLOR_BLACK);
      const splittedDescription = pdf.splitTextToSize(item.description, MAX_HORIZONTAL_WIDTH - 10);
  ...
  private addPageHeader(pdf: jsPDF) {
    ...
    pdf.text(this.generalData.name, PAGE_BORDER_LEFT, 20);
    pdf.setFont(STANDARD_FONT, 'normal');
    pdf.setFontSize(18);
    pdf.setTextColor(68, 68, 68);
    pdf.text(this.generalData.position, PAGE_BORDER_LEFT, 30);
    pdf.setDrawColor(68, 68, 68);
    pdf.line(PAGE_BORDER_LEFT, 35, 188, 35); // horizontal line
    this.verticalPosition = 50;
    pdf.setTextColor(TEXT_COLOR_BLACK);
  }

  private addPageFooter(pdf: jsPDF) {
    pdf.line(PAGE_BORDER_LEFT, 280, 188, 280); // horizontal line
    pdf.setFont(STANDARD_FONT, 'normal');
    pdf.setFontSize(7);
    pdf.text(this.l10n.pdf.info_generation, PAGE_BORDER_LEFT, 284);
    pdf.setFontSize(7);
    pdf.text(this.l10n.pdf.info_headhunter, PAGE_BORDER_LEFT, 287);
